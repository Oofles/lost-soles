...
> **Corrected 2026-09-02 (capability `02` close audit, D-176).** The tree below was written
> before `06-ui-ux.md` existed and **is not what shipped.** Where the two disagree,
> **`06-ui-ux.md` §1.2 is normative for the information architecture** — it is the document
> that reasoned about the IA, and §4.1 of it settles the biggest difference: *there is no map
> screen.* The map is the home screen.
>
> What ticket `0016` actually built, and what `middleware.ts` gates, is the seven routes of
> §1.2 — nine App Router segments, no route groups:
>
> ```
> app/
>   layout.tsx                  root: tokens, <AuthGate>
>   page.tsx                    / — map + plinth (the home screen; no /map route)
>   skills/page.tsx             /skills
>   skills/[skillId]/page.tsx   sheet over /skills, a route so back works
>   log/page.tsx                /log — add workout (D-061)
>   chronicle/page.tsx          sheet over /, a route so back works
>   run/[activityId]/page.tsx   /run/:id — the post-run moment
>   settings/page.tsx           /settings
>   dev/tickets/page.tsx        /dev/tickets (D-092)
>   dev/tickets/[id]/page.tsx
>   api/tickets/capture/route.ts
> ```
>
> Specifically absent, and deliberately so: **no `(public)`/`(app)` route groups** and **no
> `/sign-in` route** — `/` doubles as the signed-out landing so §1.5's "back always returns
> toward `/`" stays true; **no `/map`**; **no `/activities`** — that is `/chronicle`; **no
> `settings/sources/`** — Strava re-auth is a row inside `/settings`. The auth gate is
> `middleware.ts` (server-side, 0016), not an `(app)/layout.tsx` server component; it covers
> every non-static route including `/api/*`, which is the ticket-capture endpoint's only
> authentication today (07 §6.4, ticket `0019` hardens it).
>
...
`src/domain/` is shared between the Next.js app and `amplify/functions/*`. Both are
TypeScript, both bundle with esbuild, so the XP rules and the H3 helpers exist **once**.
(**Corrected 2026-09-02, D-176:** this paragraph and the tree above said `lib/domain/`,
contradicting **§3's module layout in this same document** — `src/domain/`, `src/pipeline/`,
`src/adapters/` — which is what ticket `0025` built and what `scripts/check-boundaries.mjs`
enforces. §3 is right; `lib/` holds app-only helpers.) A
...
> **Corrected 2026-09-02 (capability `02` close audit, D-176).** The YAML above is the
> *intent*, not the shipped file. Two things diverged and both are deliberate:
>
> 1. **Install is `npm install --no-save`, not `npm ci`** — in **both** phases, and in
>    `.github/workflows/gate.yml` too. `npm ci` cannot install the Amplify Gen 2 dependency
>    set at all; the full evidence is **D-162**. This is a temporary retreat with a revert
>    ticket attached (**`0128`**, `deferred` on an upstream fix), not a new preference. The
>    lock guarantee `npm ci` bought is replaced by an explicit
>    `git diff --exit-code package-lock.json` step.
> 2. **The build phase carries six guards this block omits**, added by later tickets in the
>    same capability. In `backend`: `check-auth-posture.mjs` after `pipeline-deploy` (0014).
>    In `frontend.build`, before `npm run build`: `check-boundaries.mjs` (D-100),
>    `check-design-tokens.mjs` (06 §8), and `npm test`; after it,
>    `check-bundle-leak.mjs --self-test` and `--require-literals` (0017/0132).
>
> **Read `amplify.yml` itself as the authority on the deploy path**, not this block. It is
> the LOCK (D-163) — there is no branch protection on this repo, so it is the only thing
> that can stop bad code reaching `soles.devaultsecurity.com`, and every command in it is
> commented with the ticket and decision that put it there.
