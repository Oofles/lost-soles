...
- [ ] Zooming in by several levels rebuilds the buffer rather than keeping one sized for a much
      larger area.
- [ ] `visibleInstanceCount` at z15-z17 reflects those viewports rather than repeating z13's number.
- [ ] A small zoom change still does NOT rebuild — §6.2's whole point is that a pinch does not cost a
      cull per frame, and `viewport-controller.test.ts`'s zero-upload assertions must survive.
- [ ] The debounce still collapses a fast pinch into two or three rebuilds (§6.1).
- [ ] `tools/fog-harness/run-perf.mjs`'s `pan-z17` phase performs culls, which it cannot today.
